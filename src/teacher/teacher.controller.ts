...
export class TeacherController {
  ...
  @Get('me/invite-link')
  @ApiOperation({ summary: 'Получить постоянную ссылку для приглашения учеников' })
  ...
  updateProfile(@Request() req: any, @Body() body: { displayName?: string; bio?: string }) {
  ...
  createSubject(@Request() req: any, @Body() body: { name: string; code?: string; colorHex: string }) {
  ...
  updateSubject(@Request() req: any, @Param('subjectId') subjectId: string, @Body() body: { name?: string; colorHex?: string }) {
  ...
  @ApiOperation({ summary: 'Создать приглашение для ученика (ссылка для регистрации)' })
  ...
  getStudentDetails(@Request() req: any, @Param('studentId') studentId: string) {
  ...
  updateStudent(@Request() req: any, @Param('studentId') studentId: string, @Body() body: { customFields: Record<string, string> }) {
  ...
  createParentInvitation(@Request() req: any, @Param('studentId') studentId: string) {
  ...
  getStudentParents(@Request() req: any, @Param('studentId') studentId: string) {
  ...
  @ApiQuery({ name: 'from', required: true, description: 'Начало периода (ISO date)', example: '2025-01-01' })
  @ApiQuery({ name: 'to', required: true, description: 'Конец периода (ISO date)', example: '2025-12-31' })
  ...
  getLessons(
    ...
    return this.teacherService.getLessons(req.user.profileId, from, to, { subjectId, studentId, status })
  }
  ...
  @ApiQuery({ name: 'applyToSeries', required: false, enum: ['this', 'future', 'all'] })
  updateLesson(
    @Request() req: any,
  ...
  @ApiQuery({ name: 'applyToSeries', required: false, enum: ['this', 'future', 'all'] })
  deleteLesson(@Request() req: any, @Param('lessonId') lessonId: string, @Query('applyToSeries') applyToSeries?: string) {
  ...
  @ApiOperation({ summary: 'Получить краткую статистику ученика (для карточки)' })
  ...
  getStudentDetailedStats(@Request() req: any, @Param('studentId') studentId: string) {
