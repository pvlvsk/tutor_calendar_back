 ...
 * @description Тесты для функций расчёта статистики
 * @relatedTo ../stats.service.ts
 *
 * Покрывает: calculateAttendanceFromRecords, calculateStatsBySubject, calculateStatsByTeacher
 */
...
// Эмулируем private методы из StatsService
function calculateAttendanceFromRecords(records: LessonStudent[]): AttendanceStats {
...
function calculateStatsBySubject(records: LessonStudent[]): SubjectStats[] {
...
function calculateStatsByTeacher(records: LessonStudent[]): TeacherStats[] {
  const byTeacher = new Map<string, { name: string; attended: number; total: number }>();
