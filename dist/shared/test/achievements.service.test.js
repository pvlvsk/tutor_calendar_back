function calculateAchievementsFromRecords(records, streak) {
    const attended = records.filter((r) => r.lesson?.status === 'done' && r.attendance === 'attended');
    const achievements = [];
    const sortedAttended = [...attended].sort((a, b) => new Date(a.lesson.startAt).getTime() - new Date(b.lesson.startAt).getTime());
    achievements.push({
        id: 'first_lesson',
        name: 'Первый шаг',
...
const createRecord = (attendance, status, date) => ({
    attendance,
    lesson: {
...
describe('AchievementsService — calculateAchievementsFromRecords', () => {
    ...
    describe('Десятка', () => {
        it('не должен быть получен с 5 уроками', () => {
            const records = Array.from({ length: 5 }, (_, i) => createRecord('attended', 'done', `2025-01-${15 + i}T10:00:00`));
        ...
        it('должен быть получен после 10 посещённых уроков', () => {
            const records = Array.from({ length: 10 }, (_, i) => createRecord('attended', 'done', `2025-01-${10 + i}T10:00:00`));
        ...
        it('progress не должен превышать 10', () => {
            const records = Array.from({ length: 15 }, (_, i) => createRecord('attended', 'done', `2025-01-${10 + i}T10:00:00`));
    ...
    describe('Пять подряд', () => {
        it('не должен быть получен при streak < 5', () => {
            const records = Array.from({ length: 4 }, (_, i) => createRecord('attended', 'done', `2025-01-${15 + i}T10:00:00`));
    ...
    describe('Идеальная неделя', () => {
        ...
        it('должна быть получена при 3+ уроках без пропусков', () => {
            const records = [
                createRecord('attended', 'done', '2025-01-13T10:00:00'),
                createRecord('attended', 'done', '2025-01-14T10:00:00'),
                createRecord('attended', 'done', '2025-01-15T10:00:00'),
            ];
        ...
        it('не должна быть получена если есть пропуск', () => {
            const records = [
                createRecord('attended', 'done', '2025-01-13T10:00:00'),
                createRecord('attended', 'done', '2025-01-14T10:00:00'),
                createRecord('missed', 'done', '2025-01-15T10:00:00'),
            ];
            const achievements = calculateAchievementsFromRecords(records, 2);
            const perfectWeek = achievements.find((a) => a.id === 'perfect_week');
            expect(perfectWeek?.progress).toBe(0);
...
//# sourceMappingURL=achievements.service.test.js.map